...
const Calendar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await laravelAxios.get(`${apiUrl}/api/log/getAll`, {
          cache: 'no-store',
        })
        const formattedData = {}
        for (const event of res.data.logs) {
          if (event.updated_at) {
            const date = new Date(
              new Date(event.updated_at).getTime() - 9 * 60 * 60 * 1000,
            )
        ...
        <div className="border p-2">
          {events.map((event, index) => {
            if (event.stock_name && event.memo && event.memo_title) {
              const localUpdatedAt = new Date(event.updated_at)
                .toLocaleDateString('ja-JP', { timeZone: 'Asia/Tokyo' })
                .replace(/\//g, '-')
                .replace(/-(\d)-/g, '-0$1-')
                .replace(/-(\d)$/, '-0$1') // 月と日を2桁にする
              let editorState
              try {
                ...
                <div
                  ...
                  <div
                    ...
                    <p className="break-words">
                      {event.stock_name}({event.stock_id})
                    </p>
                    <p>
                      <strong>タイトル:</strong>
                    </p>
