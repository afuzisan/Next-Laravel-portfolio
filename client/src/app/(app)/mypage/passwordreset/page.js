...
const Component = () => {
    ...
    const fetchData = async (data) => {
        try {
            const response = await laravelAxios.post(`${apiUrl}/api/mypage/passwordReset`, {
                password: data
            });
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black z-50">
                    <div className="bg-white p-6 rounded shadow-lg justify-around flex items-center">
                        ...
                        <button onClick={handlePasswordReset} className="bg-red-500 text-white py-2 px-4 rounded m-2">変更</button>
                    </div>
                </div>
            )}
        </div>
