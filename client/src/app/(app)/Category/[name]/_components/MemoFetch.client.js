...
const MemoFetch = ({
  ...
  useEffect(() => {
    if (result && onDataResult) {
      ...
      result.stocks.forEach(stock => {
        initialCategories[stock.stock_code] =
  ...
  const handleLog = async stockCode => {
    try {
      const response = await laravelAxios.get(
        `${apiUrl}/api/log/getStockLog?stockCode=${stockCode}`,
      )
  ...
  const handleDateChange = (e, stockCode) => {
    ...
    setSelectedDates(prevDates => ({ ...prevDates, [stockCode]: newDate }))
    handleImageClick(stockCode, false, newDate)
  }
  ...
  useEffect(() => {
    ...
    fetchData()
  }, [MemoRefreshKey, sortOrder, currentPage, param])
              ...
              <div className="grid grid-cols-[1fr_3fr_3fr] h-[320px]">
                ...
                <div className="grid-item pl-2 relative">
                  ...
                  <input
                    ...
                    id="start"
                    name="trip-start"
                    value={`${selectedDates[stock.stock_code] || formatDate(new Date(), '-', 0)}`}
                    min="2018-01-01"
...
const initFetch = async (
  ...
  try {
    const result = await laravelAxios.get(
      `${apiUrl}/api/Categories/index?param=${param}&page=${itemsPerPage}&category=${params.name}`,
      { cache: 'no-cache' },
    )
