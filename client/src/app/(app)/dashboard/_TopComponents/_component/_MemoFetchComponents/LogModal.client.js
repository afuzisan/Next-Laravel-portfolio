...
const LogModal = ({ modalIsOpen, closeModal, modalContent, resultStocks }) => {
  ...
  const initStockLog = async apiUrl => {
    try {
      const response = await laravelAxios.get(
        `${apiUrl}/api/log/getStockLog?stockCode=${modalContent.StockCode}`,
      )
  ...
  const handleFormattedDate = (
    ...
    handleImageClick(
      ...
      memoTitle,
      false,
      formattedDate.replace(/-/g, '').slice(2),
      currentCount,
    )
  }
  ...
  const getStockInfo = () => {
    if (modalContent?.memo_logs?.length > 0 && resultStocks) {
      let stock = resultStocks.find(
        stock =>
          Number(stock.id) === Number(modalContent.memo_logs[0].stock_id),
      )
      if (!stock) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap justify-start mt-6">
        {modalContent.memo_logs &&
          modalContent.memo_logs.map((log, index) => {
              ...
              >
                <div className="relative">
                  <img
                    ...
                    src={
                      chartImages[
                        `${getStockInfo().stock_code}-${imageFormattedDate}-${log.memo_title}`
                      ] ||
                      `https://www.kabudragon.com/chart/s=${getStockInfo().stock_code}/e=${selectedDates[`${getStockInfo().stock_code}-${imageFormattedDate}-${log.memo_title}`]?.replace(/-/g, '').slice(2) || imageFormattedDate}.png`
                    }
                    alt="Stock Image"
                    onClick={e => {
                      e.stopPropagation()
                      handleImageClick(
                        getStockInfo().stock_code,
                        imageFormattedDate,
                        1,
                        log.memo_title,
                        false,
                  ...
                  />
                  <span className="border absolute top-[-3px] left-0 text-black bg-white p-1 rounded truncate max-w-[200px]">
                    {getStockInfo().stock_name}({getStockInfo().stock_code})
                  </span>
