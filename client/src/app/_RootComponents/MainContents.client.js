    ...
    >
      {['迅速な銘柄登録・管理', 'スマートなカテゴリ分類', '柔軟なソート機能', '直感的な操作性'].map((item, index) => (
        <motion.li
          ...
          className="text-lg text-gray-700 flex w-96"
        >
...
const MainContents = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setCurrentContent('video'), 3000);
    return () => clearTimeout(timer);
  }, []);

  return (
    <div className="relative">
      {currentContent === 'intro' ? <IntroContent /> : <VideoContent />}
      <button
        onClick={() => setCurrentContent(currentContent === 'intro' ? 'video' : 'intro')}
        className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors"
      >
